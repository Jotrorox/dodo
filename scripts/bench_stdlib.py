...
def main():
    ...
    if any(size < 2 for size in args.route_sizes):
        parser.error("route-sizes must be at least 2")
        ...
        with tempfile.TemporaryDirectory(prefix="dodo-stdlib-bench-") as directory:
            ...
            if args.suite in ("web", "all"):
                ...
                for name, mode, expected in (
                    ("web.headers.indexed.32", 0, lambda n: n * 32 * 13),
                    ("web.headers.sequential.32", 1, lambda n: n * 32 * 13),
                    ("web.parameters.uncached", 2, lambda n: n // 3 * 9 + (0, 4, 6)[n % 3]),
                    ("web.parameters.captured", 3, lambda n: n // 3 * 9 + (0, 4, 6)[n % 3]),
                    ("web.path.checked", 4, lambda n: n),
                    ("web.path.validated", 5, lambda n: n),
                ):
