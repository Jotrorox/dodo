...
| Syntax | Newline statement termination, braced blocks, mandatory function return types, explicit returns, type-first declarations, inferred mutable locals, constants, struct-local methods. |
...
| Literals | Contextual integer inference with isize default; typed integers; byte, string, byte-string, and array literals; immutable program-lifetime string storage. |
...
| Matching | Exhaustive statement with no fallthrough; literals/variants/payload bindings/wildcard; consume an owned non-copy scrutinee and borrow payloads of reference scrutinees. |
...
operator precedence/evaluation, generic inference, patterns, entry conventions,
