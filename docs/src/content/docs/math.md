...
The split ln(2) constants, reciprocal, exponential cutoffs, and all 66 digits of
2/pi are independently generated by `scripts/generate_math_constants.py` using
integer bounds on elementary series at 2,048 and 4,096 bits. Both bounds must
produce the same digits and binary64 rounding, and both precision settings must
agree. Run `python3 scripts/generate_math_constants.py --check` to verify them
or `--write` to regenerate them; only Python's standard library is needed.
The derivations and replacement provenance are recorded in
`stdlib/std/math/CONSTANTS.md`. These independently implemented kernels have no
uniform one-ulp accuracy guarantee. The table reduction follows the established
Payne–Hanek method described
