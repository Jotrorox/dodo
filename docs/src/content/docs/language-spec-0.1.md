...
`llvm.trap` by default on freestanding targets. Compiler options can select a C
ABI panic hook or trap-only behavior; a returning hook is followed by `llvm.trap`.
